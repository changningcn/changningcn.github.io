...
# 例如导出所有数据库：
mongodump /usr/changning/mongodb/
```
...
# 例如恢复所有数据库：	
    ...
    mongorestore /usr/changning/mongodb/	

# 恢复指定的数据库
    
    mongorestore -d test /usr/changning/mongodb/test/
```
...
# 例如导出表 visit_data
    ...
    mongoexport  -d smarket -c visit_vector  -o /usr/changning/mongodb/smarket/visit_vector.dat	

# 导出部分字段 _id,school,teacher

    mongoexport -d smarket -c visit_vector --type=csv -f _id,school,teacher -o /usr/changning/mongodb/smarket/visit_vector.csv
```
