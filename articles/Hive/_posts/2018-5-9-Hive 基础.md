...
```
### 特性
当写入类型与数据库中不匹配时，如果可以自动判断(比如 ```int``` 类型 写入了 ```'19'```就会自动转化成 ```int```类型插入，如果不能识别，会存一个```NULL```)
<center>
<div>
<img src="/articles/Hive/images/hive_01.png"/>
</div>
图 1 hive 类型不匹配
</center>

### 将 String 转化成 StructType scala 代码
```scala
def buildStructType(fields: String): StructType = {
    try{
      StructType(fields.replace("\n","").split(",").map(_.split(":"))
        .map(field => {
          val fieldType = field(1).toString.toLowerCase().trim match {
            ...
            case "calendarinterval" => CalendarIntervalType
            case _ => StringType
      ...
      case e : Exception => throw new RuntimeException("please input right schema, and Exception is:   " +  e)
    }
  }
```