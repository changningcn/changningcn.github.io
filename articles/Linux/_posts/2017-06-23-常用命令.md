...
# 对于每一类用户的权限又分为三种
  1)可读 r ，权重是2的2次方 = 4
  2)可写 w ，权重是2的1次方 = 2 (对于文件夹: 能否对内部文件进行操作(删除、重命名等))
  3)可执行 x ，权重是2的0次方 = 1 (对于文件夹: 能否访问)

# 改变文件所属用户和组  -R 代表递归更改文件夹内的所有文件
  chown -R changning:changning directory1
  # 只改变文件所属用户
  chown changning file.txt
  # 只改变文件所属组
  chgrp changning file.txt
...
$ scp -rp /usr/changning/source/. user@server2:/usr/changning/test/
...
$ tar -zxvf spark-2.0.2-bin-hadoop2.6.tgz -C /usr/changning/spark 
...
$ kill %3 # 将第3个job干掉
# nohup = no hang up 不挂断，如果你正在运行一个进程，而且你觉得在退出帐
# 户时该进程还不会结束，那么可以使用nohup命令。该命令可以在你退出帐户/关闭
# 终端之后继续运行相应的进程.长命令必须写在shell文件中，否则 nohup 不起作用
# 该命令的一般形式 
$ nohup command &
